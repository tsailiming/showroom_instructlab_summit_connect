...
== Integrating the Model with an Insurance Application
...
Let's start by taking a look at the current experience a claims agent has when interacting with the chatbot. You should see the Parasol company's claims application in your browser.

 Open a browser on your system and navigate to the following URL replace GUID and ID with the appropriate values for your environment.
