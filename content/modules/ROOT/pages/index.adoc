...
[source,sh]
----
(venv) $ pip3 install git+https://github.com/instructlab/instructlab.git@stable
...
[source,sh]
----
(venv) $ ilab download --repository instructlab/granite-7b-lab-GGUF --filename=granite-7b-lab-Q4_K_M.gguf
----

The ilab download command downloads a model from the HuggingFace instructlab organization that we will use for this workshop. To avoid conference internet issues, we have already placed the model onto your machine.  The download command sees it and completes immediately.

The output should look like the following:

NOTE: *This command may not show the contents below due to the model being cached on the local machine.*
...
[source,sh]
----
(venv) $ ilab serve --model-path ./models/granite-7b-lab-Q4_K_M.gguf
----
...
[source,sh]
----
(venv) $ ilab chat -m models/granite-7b-lab-Q4_K_M.GGUF
----
...
[source,texinfo]
----
drwxr-xr-x@  7 gshipley  staff  224 Apr 24 18:25 .
drwxr-xr-x  18 gshipley  staff  576 Apr 24 15:29 ..
-rw-r--r--@  1 gshipley  staff  554 Apr 24 09:04 config.yaml
drwxr-xr-x@  3 gshipley  staff   96 Apr 24 09:39 data
drwxr-xr-x@  3 gshipley  staff   96 Apr 24 18:18 models
drwxr-xr-x@ 21 gshipley  staff  672 Apr 24 09:04 taxonomy
drwxr-xr-x@  7 gshipley  staff  224 Apr 24 09:02 venv
...
[source,sh]
----
(venv) $ mkdir -p ~/instructlab/taxonomy/knowledge/instructlab/overview
----
...
[source,sh]
----
(venv) $ cp -av ~/files/qna.yaml ~/instructlab/taxonomy/knowledge/instructlab/overview
----
...
[source,sh]
----
(venv) $ head ~/instructlab/taxonomy/knowledge/instructlab/overview/qna.yaml
----
...
----
(venv) $ cd ~/instructlab
(venv) $ ilab serve --model-path ./models/merlinite-7b-lab-Q4_K_M.gguf
----
...
[source,sh]
----
(venv) $ ilab generate --num-instructions 5
----
...
[source,sh]
----
(venv) $ ilab serve --model-path ~/instructlab/models/ggml-model-Q4_K_M.gguf
----
...
[source,sh]
----
(venv) $ ilab chat -m models/ggml-model-Q4_K_M.gguf
----
